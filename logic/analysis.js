...
export function analyze(candles, options = {}) {
  ...
  if (!candles || candles.length < cfg.minCandles) {
    return {
      ok: false,
      error: `حداقل ${cfg.minCandles} کندل معتبر لازم است (تعداد فعلی: ${candles ? candles.length : 0}).`
    };
  }
  ...
  if (clean.length < cfg.minCandles) {
    return {
      ok: false,
      error: `پس از پالایش داده، حداقل ${cfg.minCandles} کندل معتبر لازم است (تعداد فعلی: ${clean.length}).`
    };
  }
  ...
  // Persian report (UI-facing)
  let leadFa;
  if (signal === 'BUY') leadFa = 'شواهد فعلی بیشتر از سناریوی صعودی حمایت می‌کند.';
  else if (signal === 'SELL') leadFa = 'شواهد فعلی بیشتر از سناریوی نزولی حمایت می‌کند.';
  else leadFa = 'سیگنال‌ها متناقض‌اند و برتری واضحی دیده نمی‌شود.';
  ...
  const trendFa = trendFaMap[trend] || trend;
  const report = `${leadFa} روند: ${trendFa}. تعداد کندل: ${clean.length}.`;

  // Actionable suggestion (max ~2 lines), derived only from score/signal/risk/S-R
...
/**
 * Build a short, calculation-backed suggestion in Persian.
 * Never invents trades without signal backing.
 */
function buildSuggestion(ctx) {
  const { signal, score, riskLevel, price, support, resistance, volPct, brk } = ctx;
  const highRisk = riskLevel === 'High' || (volPct != null && volPct > 7);
  const nearRes = resistance != null && price > 0 && (resistance - price) / price < 0.02;
  const nearSup = support != null && price > 0 && (price - support) / price < 0.04;

  if (signal === 'BUY') {
    if (highRisk) {
      return 'سیگنال خرید وجود دارد، اما ریسک بالاست. حجم معامله را محدود کنید و حد ضرر را رعایت کنید.';
    }
    if (nearRes) {
      return 'روند صعودی است؛ با این حال قیمت نزدیک مقاومت است. برای ورود صبر یا شکست مقاومت را در نظر بگیرید.';
    }
    if (brk && brk.up) {
      return 'شکست صعودی همراه با امتیاز خرید است. می‌توان ورود تدریجی با حد ضرر زیر حمایت اخیر را بررسی کرد.';
    }
    return 'شرایط برای خرید مناسب‌تر است. ورود با مدیریت ریسک و تعیین حد ضرر پیشنهاد می‌شود.';
  }

  if (signal === 'SELL') {
    if (highRisk) {
      return 'سیگنال فروش همراه با ریسک بالاست. از موقعیت‌های خرید پرریسک فاصله بگیرید و حد ضرر را جدی بگیرید.';
    }
    if (nearSup) {
      return 'فشار نزولی دیده می‌شود؛ قیمت نزدیک حمایت است. برای فروش عجله نکنید تا واکنش حمایت مشخص شود.';
    }
    if (brk && brk.down) {
      return 'شکست نزولی تأیید شده است. کاهش یا بستن موقعیت‌های خرید و رعایت حد ضرر منطقی است.';
    }
    return 'شواهد به نفع فروش یا کاهش موقعیت خرید است. صبر بدون برنامه توصیه نمی‌شود.';
  }

  // HOLD
  if (highRisk) {
    return 'بازار نامشخص و پرنوسان است. فعلاً صبر کنید و از ورود عجولانه خودداری کنید.';
  }
  if (nearSup) {
    return 'سیگنال خنثی است و قیمت نزدیک حمایت قرار دارد. تا روشن‌شدن جهت، بررسی بیشتر و صبر مناسب‌تر است.';
  }
  if (nearRes) {
    return 'سیگنال خنثی و قیمت نزدیک مقاومت است. برای تصمیم قطعی، شکست یا برگشت از مقاومت را دنبال کنید.';
  }
  if (score >= 55 && score < 65) {
    return 'امتیاز کمی به سمت خرید متمایل است اما کافی نیست. منتظر تأیید بیشتر بمانید.';
  }
  if (score <= 45 && score > 35) {
    return 'امتیاز کمی به سمت فروش متمایل است اما کافی نیست. فعلاً صبر و رصد بهتر است.';
  }
  return 'شرایط برای تصمیم قطعی کافی نیست. صبر و بررسی داده‌های جدید منطقی‌ترین اقدام است.';
}
